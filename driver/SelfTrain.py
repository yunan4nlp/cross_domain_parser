...
def train(data, dev_data, test_data, unlabelled_data, parser, classifier, charEmbedding, vocab, char_vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        train_data = data
        if iter >= config.start_selftraining:
            labelled_data = labelling(unlabelled_data, parser, charEmbedding, vocab, char_vocab, config.threshold)
            print("start selftraing,  unlabelled data: ", len(labelled_data))
            train_data = data + labelled_data
...
def good_tree(tree, threshold):
    count = 0
    for dep in tree:
        if dep.score > threshold:
            count += 1
    if count / len(tree) > 0.5:
        return True
    else:
        return False

def labelling(data, parser, charEmbedding, vocab, char_vocab, threshold):
    parser.model.eval()
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        ...
        count = 0
        charEmbedding.forward(chars, char_masks)
        arcs_batch, rels_batch, arcs_values = parser.parse(words, extwords, tags, charEmbedding.outputs, lengths, masks, True)
        for tree in batch_variable_depTree(onebatch, arcs_batch, rels_batch, lengths, vocab, arcs_values):
            # arc_total, arc_correct, rel_total, rel_correct = evalDepTree(tree, onebatch[count])
            count += 1
            if good_tree(tree, threshold):
                labelled_data.append(tree)
    labelled_data = domain_labeling(labelled_data, False)
...
if __name__ == '__main__':
    ...
    if args.use_pretrain:
        ...
        classifier_model.state_dict(torch.load(config.load_classifier_model_path))
        char_emb_model.state_dict(torch.load(config.load_char_model_path))
    ...
    unlabelled = read_corpus(config.unlabelled_file, vocab)

    source_data = domain_labeling(data, True)
    target_data = domain_labeling(data_target, False)

    unlabelled_data = domain_labeling(unlabelled, False)
    ...
    train(source_data, dev_data, test_data, unlabelled_data, parser, classifier, charEmbedding, vocab, char_vocab, config)
