...
if __name__ == '__main__':
    ...
    if args.use_pretrain:
        ...
        classifier_model.load_state_dict(torch.load(config.load_classifier_model_path))
    ...
    labelled_target = read_corpus(config.labelled_file, vocab)

    source_data = domain_labeling(data, True)
    target_data = domain_labeling(data_target, False)
    labelled_data = domain_labeling(labelled_target, False)

    source_data.extend(target_data)
    print("labelled data: ", len(labelled_data))
    source_data.extend(labelled_data)
